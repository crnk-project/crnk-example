...
public class ExampleApplicationTest {
    ...
    @Test
    public void testSingularNestedResource() {
        ResourceRepository<Screening, Object> screeningRepo = client.getRepositoryForType(Screening.class);
        Screening screening = screeningRepo.findAll(new QuerySpec(Screening.class)).get(0);

        ResourceRepository<ScreeningStatus, UUID> statusRepo = client.getRepositoryForType(ScreeningStatus.class);

        ScreeningStatus status = new ScreeningStatus();
        status.setScreeningId(screening.getId());
        status.setDescription("Hello");
        statusRepo.create(status);

        ScreeningStatus newStatus = statusRepo.findOne(screening.getId(), new QuerySpec(ScreeningStatus.class));
        Assert.assertEquals("Hello", newStatus.getDescription());
    }
